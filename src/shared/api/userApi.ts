...
export const userService = {
    ...
    getFollowers: (id: number, skip: number, limit: number): Promise<UserListResponse> => {
        return baseFetch(`/users/${id}/followers?skip=${skip}&limit=${limit}`)
    },
    getFollowing: (id: number, skip: number, limit: number): Promise<UserListResponse> => {
        return baseFetch(`/users/${id}/following?skip=${skip}&limit=${limit}`)
    },

    getPosts: (id: number, skip: number, limit: number): Promise<PostsResponse> => {
