...
export const WorkoutDetails = () => {
    ...
    const displayData = workoutData?.exercises?.length ? workoutData.exercises : mockExercises;
    ...
    if (!isLoading && (!displayData || displayData.length === 0)) {
        return (
            <Container>
                <Card variant="pink">
                    <Typography variant="body">No exercises yet. Add some to start!</Typography>
                </Card>
            </Container>
        );
    }
    ...
    return (
        <Container >
            <Typography variant="h3" className={css.nxtWrktLabel}>Exercises Today </Typography>
            <ul className={css.ulContainer}>
                {displayData.map((exercise: ExerciseDataProps) => (
                    <li key={exercise.id} className={css.liExerciseItem}>
                        <WorkoutItem data={exercise} onDeleteExercise={deleteExercise} isDeleting={isPending}></WorkoutItem>
                    </li>
                ))}
            </ul>
        </Container>
